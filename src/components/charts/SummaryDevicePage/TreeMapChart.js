...
export const renderTreeMapChart = (summaryData, summaryDataValues, titleText) => {
  const uniqueDatatypes = Array.from(new Set(summaryData.device));
  ...
  const data = uniqueDatatypes.map((datatype) => ({
    ...
    value: summaryDataValues
      .map((_, index) => (summaryData.device[index] === datatype ? parseInt(summaryDataValues[index], 10) : 0))
      .reduce((a, b) => a + b, 0),
    children: uniqueStudies.map((study, index) => ({
      name: study,
      value:
        summaryData.device[index] === datatype ? parseInt(summaryDataValues[index], 10) : 0,
