...
const StudyTimeline = ({ studies, currentDate }) => {
  ...
  const today = currentDate ? new Date(currentDate) : new Date(); // This will be our "today" line
  const startDate = new Date(new Date().getTime() - 52 * 7 * 24 * 60 * 60 * 1000); // 52 weeks ago
  const endDate = new Date(new Date().getTime() + 52 * 7 * 24 * 60 * 60 * 1000); // 52 weeks in the future
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        return `
          ...
          <div>Duration: ${Math.ceil((study.end - study.start) / (7 * 24 * 60 * 60 * 1000))} weeks</div>
        `;
      }
    }
  };
